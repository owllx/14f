@echo off
rem Збирання XprinterLabelDesigner.exe на Windows (потрібен Python 3.10+ з python.org).
...
python -m PyInstaller --noconfirm --onefile --windowed --name XprinterLabelDesigner ^
  --hidden-import barcode.writer --collect-submodules barcode --collect-data barcode ^
  %EXTRA% label_designer.py
echo.
echo Готово: dist\XprinterLabelDesigner.exe
pause
